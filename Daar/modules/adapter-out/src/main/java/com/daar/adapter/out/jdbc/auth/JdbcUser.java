...
public class JdbcUser implements UserRepository {
    ...
    public User update(User user) {
        String sql = "UPDATE users SET firstname=?, lastname=?, origin=?, identityType=?, identityNumber=?, address=?, email=?, phone=?, createdAt=?, updatedAt=?, suspendedUntil=?, createdBy=?, updatedBy=?, suspendedBy=? WHERE id=?";
